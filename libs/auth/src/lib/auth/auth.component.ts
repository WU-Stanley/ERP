    ...
    this.authService.login(value).subscribe(
      (res) => {
        ...
        if (res.status) {
          ...
          const loginPayload = response?.data?.data ? response.data : response;

          this.res = loginPayload;
          this.verifyForm.patchValue({
            email: value.email,
          });
          this.isProcessing = false;
          if (loginPayload?.verifyToken || loginPayload?.data?.twoFactorEnabled) {
            this.alertService.showSuccess('Enter the token sent to your email!');
            return;
          }

          if (!loginPayload?.data?.twoFactorEnabled) {
